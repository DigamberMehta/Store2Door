...
const migrateDocuments = async () => {
  try {
    ...
    for (const profile of profiles) {
      ...
      if (profile.documents.drivingLicense?.imageUrl && !profile.documents.driversLicence?.imageUrl) {
        profile.documents.driversLicence = {
          ...
          status: profile.documents.drivingLicense.isVerified ? 'verified' : 'pending',
          uploadedAt: profile.createdAt || new Date()
        };
        needsUpdate = true;
      }

      if (profile.documents.nationalId?.imageUrl && !profile.documents.idDocument?.imageUrl) {
        profile.documents.idDocument = {
          ...
          status: profile.documents.nationalId.isVerified ? 'verified' : 'pending',
          uploadedAt: profile.createdAt || new Date()
        };
        needsUpdate = true;
      }
      ...
      if (needsUpdate) {
        ...
        await profile.save();
        migratedCount++;
        console.log(`Migrated profile: ${profile.userId} (${migratedCount}/${profiles.length})`);
      }
    }
