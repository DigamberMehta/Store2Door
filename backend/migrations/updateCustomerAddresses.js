...
const migrateCustomerAddresses = async () => {
  try {
    ...
    console.log(
      `📦 Found ${customers.length} customers with addresses in database`,
    );
    ...
    for (let i = 0; i < customers.length; i++) {
      ...
      for (let j = 0; j < customer.addresses.length; j++) {
        const coordIndex =
          (i * customer.addresses.length + j) % customerCoordinates.length;
      ...
      console.log(
        `✓ Updated ${customer.addresses.length} address(es) for customer: ${customer.userId}`,
      );
      updatedCount++;
    }
    ...
    console.log(
      `📍 Coordinates used: ${customerCoordinates.length} unique LPU-area locations`,
    );
  } catch (error) {
