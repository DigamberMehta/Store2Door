          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div className="text-sm text-gray-600">
                ...
                {order.customerId?.name && <p><strong>Customer:</strong> {order.customerId.name}</p>}
              </div>
            </div>
            <div className="text-right">
              <div className="text-2xl font-bold text-gray-900">R{order.total.toFixed(2)}</div>
                  ...
                  >
                    ...
                    <div className="w-16 h-16 bg-gray-100 rounded-lg shrink-0 overflow-hidden">
                      {item.productId?.images?.length > 0 ? (
                        <img
                          src={item.productId.images.find(img => img.isPrimary)?.url || item.productId.images[0]?.url}
                          alt={item.productId.images.find(img => img.isPrimary)?.alt || item.name}
                          className="w-full h-full object-cover"
                          onError={(e) => {
                            e.target.style.display = 'none';
                            e.target.parentElement.innerHTML = '<div class="w-full h-full flex items-center justify-center text-gray-400"><svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg></div>';
                          }}
                        />
                      ) : (
                  ...
                  {getAvailableStatuses(order.status).map((status) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={updating}
                        className={`w-full px-4 py-2 ${config.color} text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium flex items-center justify-between`}
                      >
                  ...
                  {["placed", "confirmed", "preparing"].includes(order.status) && (
                    <>
                      ...
                      <button
                        ...
                        disabled={updating}
                        className="w-full px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium flex items-center justify-between"
                      >
