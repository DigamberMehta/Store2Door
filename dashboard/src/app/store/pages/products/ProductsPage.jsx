    ...
    <div className="p-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage and view your store inventory</p>
        </div>
        <Link
      ...
      {/* Filters & Search */}
      <div className="bg-white p-4 rounded-xl border border-gray-200 mb-6 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Price (Store)</th>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Retail Price</th>
              ...
              {products.length === 0 ? (
                <tr>
                  <td colSpan="7" className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center">
                      ...
                      <Link to="/store/products/add" className="text-green-600 hover:text-green-700 text-sm mt-1">Add your first product</Link>
                    </div>
                  </td>
                </tr>
              ) : (
                products.map((product) => (
                  <tr key={product._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <p className="text-sm font-medium text-gray-900">R {product.wholesalePrice?.toFixed(2) || "0.00"}</p>
                    </td>
                    <td className="px-6 py-4">
                      <p className="text-sm font-medium text-green-600">R {product.retailPrice?.toFixed(2) || "0.00"}</p>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className={`text-sm font-medium ${(product.inventory?.quantity || 0) <= 5 ? "text-amber-600" : "text-gray-900"}`}>
        ...
        {products.length > 0 && (
          <div className="px-6 py-4 border-t border-gray-200 bg-white flex items-center justify-between">
            <div className="text-sm text-gray-600">
              Showing <span className="font-medium text-gray-900">{products.length}</span> of <span className="font-medium text-gray-900">{pagination.total}</span> products
            </div>
