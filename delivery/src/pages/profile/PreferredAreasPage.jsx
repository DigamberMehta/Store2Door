    ...
    <div className="bg-black min-h-screen text-white flex flex-col">
      ...
      <div className="pt-8 px-4 pb-4 bg-zinc-900/50 backdrop-blur-xl border-b border-white/5 flex items-center gap-4 sticky top-0 z-10">
        ...
        <div>
          ...
          <p className="text-[10px] text-zinc-500 font-medium">Select locations where you want to work</p>
        </div>
      </div>
      ...
      <div className="p-4 space-y-6 flex-1">
        ...
        <div className="space-y-3">
          <label className="text-[9px] uppercase tracking-widest font-extrabold text-zinc-500 ml-1">Add Location</label>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center px-1">
            <label className="text-[9px] uppercase tracking-widest font-extrabold text-zinc-500">Selected ({selectedAreas.length})</label>
          ...
          <div className="flex flex-wrap gap-1.5">
            {selectedAreas.length > 0 ? (
              selectedAreas.map((area) => (
                ...
                >
                  <span className="text-xs font-bold text-blue-200">{area}</span>
                  <button 
              ...
              <div className="w-full py-6 text-center bg-white/5 border border-dashed border-white/10 rounded-2xl">
                ...
                <p className="text-zinc-600 text-[10px] font-medium">No areas selected yet</p>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Suggested Areas */}
        <div className="space-y-3">
          <label className="text-[9px] uppercase tracking-widest font-extrabold text-zinc-500 px-1">Nearby & Suggested</label>
          <div className="grid grid-cols-2 gap-2">
            {suggestedAreas.filter(a => !selectedAreas.includes(a)).map((area) => (
              <button
                key={area}
                onClick={() => toggleArea(area)}
              ...
              >
                ...
                <span className="text-[11px] font-bold text-zinc-400">{area}</span>
              </button>
            ))}
          </div>
        </div>
